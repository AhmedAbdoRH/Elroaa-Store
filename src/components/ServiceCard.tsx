...
export default function ProductCard({ title, description, imageUrl, price, salePrice, id, has_multiple_sizes, has_weight_pricing, price_per_kg, sale_price_per_kg, sizes }: ProductCardProps) {
  ...
  return (
    <div className="group relative bg-primary rounded-xl border border-secondary/10 overflow-hidden transition-all duration-150 hover:scale-[1.02] hover:shadow-lg shadow-sm">
            ...
            {has_weight_pricing ? (
              ...
              <>
                ...
                {pricingStrategy === 'smart-fallback' || hasMultiplePrices ? (
                  <span className="text-xs text-secondary/50">ابتداءً من</span>
                ) : (
                  <span className="text-xs text-secondary/50">سعر موحد</span>
                )}
              </>
            ) : displaySalePrice ? (
              <>
                <div className="flex items-center gap-1">
                  <span className={`font-bold text-lg sm:text-xl text-accent`}>{displaySalePrice}</span>
                  <span className={`font-bold text-lg sm:text-xl text-accent`}>ج</span>
                </div>
                {displayPrice && (
                  <div className="flex items-center gap-1">
              ...
              <>
                <div className="flex items-center gap-1">
                  <span className={`font-bold text-lg sm:text-xl text-accent`}>{displayPrice}</span>
                  <span className={`font-bold text-lg sm:text-xl text-accent`}>ج</span>
                </div>
                {pricingStrategy === 'smart-fallback' && (
                  <span className="text-xs text-secondary/50">ابتداءً من</span>
              ...
              <span className="text-sm text-secondary/50 text-right">يرجى التواصل للاستفسار عن السعر</span>
            )}
          </div>
